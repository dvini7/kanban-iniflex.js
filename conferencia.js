(function() {
    ...
    style.innerHTML = `
        ...
        #stark-kanban-panel { background: #0B1120; border: 1px solid #1e293b; border-radius: 12px; width: 280px; box-shadow: 0 10px 25px rgba(0,0,0,0.5); margin-bottom: 15px; display: none; flex-direction: column; overflow: hidden; }
        #stark-kanban-header { background: linear-gradient(180deg, rgba(0,240,255,0.15) 0%, rgba(0,0,0,0.2) 100%); border-bottom: 3px solid #00f0ff; padding: 12px; font-size: 11px; font-weight: 900; color: #00f0ff; text-transform: uppercase; display: flex; justify-content: space-between; }
        #stark-kanban-body { padding: 10px; max-height: 400px; overflow-y: auto; display: flex; flex-direction: column; gap: 8px; }
        .stark-card { background: #1e293b; border-radius: 8px; padding: 10px; border-left: 3px solid #00f0ff; display: flex; flex-direction: column; gap: 6px; position: relative; }
        ...
        #btn-add-pedido { background: rgba(0, 240, 255, 0.1); border: 1px dashed #00f0ff; color: #00f0ff; width: 100%; padding: 10px; border-radius: 8px; font-weight: bold; cursor: pointer; margin-bottom: 10px; }
        #btn-add-pedido:hover { background: #00f0ff; color: #000; }
        .bolinha-status { width: 10px; height: 10px; border-radius: 50%; position: absolute; top: 10px; right: 10px; }
    `;
    ...
    async function renderizarLista() {
        try {
            ...
            divLista.innerHTML = '<span style="color:red; font-size: 11px; padding: 10px;">Erro ao conectar com o servidor.</span>';
        }
    }
